...
public class KbSearchClientImpl implements KbSearchClient {
  ...
  public KbSearchClientImpl() {
    mockChunks.put("退款", "退款申请需在订单签收后 7 天内提交，审核通过后将原路退回支付账户。");
    mockChunks.put("退货", "如需退货，请在确认收货后 7 天内发起退货申请，商品需保持原包装完好。");
    mockChunks.put("发票", "电子发票会在订单完成后 24 小时内发送至您的注册邮箱，可在订单详情页下载。");
    mockChunks.put("物流", "您可以在订单详情页查看实时物流信息，通常 1-3 个工作日送达。");
  }
  ...
  public List<KbChunk> search(SearchRequest request) {
    ...
    for (Map.Entry<String, String> entry : mockChunks.entrySet()) {
      if (query != null && query.contains(entry.getKey())) {
        return List.of(new KbChunk(entry.getValue(), 0.85, "DOC-MOCK-0001", "知识库文档.txt"));
      }
    }
    return List.of(new KbChunk("暂无与该查询直接相关的文档内容。", 0.0, "DOC-MOCK-0001", "知识库文档.txt"));
  }
}
